...
const generateChronosConfig = {
  ...
  description: process.env.CHRONOS_DESCRIPTION || "A frontend in Vue.js for the Chronos documentation server.",
...
export async function createChronosPath() {
  try {
    ...
    if (process.env.CHRONOS_LOGO_URL && !isWebURL(process.env.CHRONOS_LOGO_URL)) {
      ...
      await fs.copyFile(process.env.CHRONOS_LOGO_URL, logoFilePath);
      generateChronosConfig.logoUrl = `/logo${logoExtension}`;
    }

    const configFilePath = resolve(publicFolderPath, "chronos.json");
    await fs.writeFile(configFilePath, JSON.stringify(generateChronosConfig, null, 2));

    console.log("chronos.json generated successfully!");
  } catch (error) {
    console.error("Failed to generate chronos.json:", error);
  }
}
